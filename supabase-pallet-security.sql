-- Additive migration: no inventory/pallet data is removed.
-- Browser writes must use this atomic, version-checked RPC, including imports.
create table public.pallet_editors (
  email text primary key check (email = lower(btrim(email))),
  created_at timestamptz not null default now()
);
alter table public.pallet_editors enable row level security;
revoke all on public.pallet_editors from public, anon, authenticated;

alter table public.pallet_slots add column version bigint not null default 1;
alter table public.receive_dates add column version bigint not null default 1;

create function public.can_edit_pallets() returns boolean
language sql stable security definer set search_path = '' as $$
  select exists (
    select 1 from auth.users u join public.pallet_editors e on e.email = lower(u.email)
    where u.id = auth.uid() and u.email_confirmed_at is not null
      and not coalesce(u.is_anonymous, false)
  );
$$;
revoke all on function public.can_edit_pallets() from public, anon, authenticated;
grant execute on function public.can_edit_pallets() to authenticated;

-- Remove ALL browser write policies on these two tables, including legacy names.
do $$ declare p record; begin
  for p in select tablename, policyname from pg_policies
    where schemaname = 'public' and tablename in ('pallet_slots','receive_dates')
      and cmd <> 'SELECT'
  loop execute format('drop policy %I on public.%I', p.policyname, p.tablename); end loop;
end $$;
revoke insert, update, delete, truncate, references, trigger
  on public.pallet_slots, public.receive_dates from public, anon, authenticated;
grant select on public.pallet_slots, public.receive_dates to anon, authenticated;

create function public.save_pallet_changes(p_slots jsonb default '[]', p_dates jsonb default '[]')
...
declare
  ...
  expected bigint;
begin
  if not public.can_edit_pallets() then
    raise exception 'PALLET_FORBIDDEN: confirmed editor account required' using errcode = '42501';
  end if;
...
grant execute on function public.save_pallet_changes(jsonb,jsonb) to authenticated;
