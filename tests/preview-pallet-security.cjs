...
http.createServer((req,res)=>{
 ...
 let fixtureMode='ok',fixtureEditor=false;
 ...
 for(const mode of ['editor','conflict','network','ok']){
 const b=document.createElement('button');b.textContent='Test '+mode;
 b.style.cssText='position:relative;z-index:999999';
 b.onclick=()=>{if(mode==='editor'){fixtureEditor=true;refreshPalletAuth()}else fixtureMode=mode};document.body.appendChild(b);
 }
 </script></body>`);
